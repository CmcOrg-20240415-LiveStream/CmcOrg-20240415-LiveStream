...
public class MyDraft extends Draft {
     ...
     *
     * @param inputExtensions the extensions which should be used for this draft
     * @param inputMaxFrameSize the maximum allowed size of a frame (the real payload size, decoded frames can be
     *            bigger)
     * @since 1.4.0
     */
    public MyDraft(List<IExtension> inputExtensions, int inputMaxFrameSize) {
     ...
     * @since 1.4.0
     */
    public MyDraft(List<IExtension> inputExtensions, List<IProtocol> inputProtocols, int inputMaxFrameSize) {
    ...
    public Draft copyInstance() {
        ...
        return new MyDraft(newExtensions, newProtocols, maxFrameSize);
    }
    ...
    private ByteBuffer createByteBufferFromFramedata(Framedata framedata) {
        ...
        ByteBuffer buf =
            ByteBuffer.allocate(1 + (sizebytes > 1 ? sizebytes + 1 : sizebytes) + (mask ? 4 : 0) + mes.remaining());
    ...
    private Framedata translateSingleFrame(ByteBuffer buffer) throws IncompleteException, InvalidDataException {
        ...
        if (!(payloadlength >= 0 && payloadlength <= 125)) {
            MyDraft.TranslatedPayloadMetaData payloadData =
                translateSingleFramePayloadLength(buffer, optcode, payloadlength, maxpacketsize, realpacketsize);
     ...
     */
    private MyDraft.TranslatedPayloadMetaData translateSingleFramePayloadLength(ByteBuffer buffer, Opcode optcode,
        ...
        return new MyDraft.TranslatedPayloadMetaData(payloadlength, realpacketsize);
    }
    ...
    public boolean equals(Object o) {
        ...
        if (negotiatedExtension != null ? !negotiatedExtension.equals(that.getExtension())
     ...
     * @throws LimitExceededException will be thrown when the totalSize is bigger then Integer.MAX_VALUE due to not
     *             being able to allocate more
     */
